...
"                   filetype plugin on
"                   let g:iRule_location = 'path/to/complete-dict'
"               Optionally, you set the completion menu height like:
"                   let g:iRule_menu_height = 20
...
if v:version < 700
    echoerr "iRule requires vim version 7 or greater."
    finish
endif
...
if !exists("*s:SetVals") 
    function! s:SetVals()
		...
		"
        let s:iRule_save_isk = &iskeyword
        "setlocal iskeyword +=.,(
        "Added ":" for iRules - MC
        setlocal iskeyword +=_,:,(

        " Save any current dictionaries the user has set:
        let s:iRule_save_dictions = &dictionary
        " Temporarily use only iRule's dictionary:
        "let &dictionary = g:iRule_location
        let &dictionary = "iRule_dict"

        " Save the ins-completion options the user has set:
        let s:iRule_save_cot = &completeopt
        ...
        let s:iRule_save_pumheight = &pumheight
        if !exists('g:iRule_menu_height')
            let g:iRule_menu_height = 15
        endif
        let &pumheight = g:iRule_menu_height
...
if !exists("*s:RestoreVals")
    function! s:RestoreVals()
        ...
        let &dictionary = s:iRule_save_dictions
        let &completeopt = s:iRule_save_cot
        let &pumheight = s:iRule_save_pumheight
        let &iskeyword = s:iRule_save_isk
